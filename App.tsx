...
const App: React.FC = () => {
  ...
  const handleStartQuiz = useCallback(async (mode: QuizMode, options?: { topics?: string[], hashtag?: string }) => {
    ...
    setQuizMode(mode);
    try {
      const generatedQuestions = await generateQuiz(mode, options);
  ...
  const renderContent = () => {
    if (isLoading) {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen">
          <LoadingSpinner />
          <p className="mt-4 text-sky-300">Generando tu cuestionario con IA...</p>
        </div>
      );
    }
    ...
    switch (appState) {
      case AppState.Home:
        return <HomeScreen onStartQuiz={handleStartQuiz} error={error} />;
      case AppState.Quiz:
  ...
  return <div className="min-h-screen bg-slate-900 text-white p-4 sm:p-6 md:p-8">{renderContent()}</div>;
};
